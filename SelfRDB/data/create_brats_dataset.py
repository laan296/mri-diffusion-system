...
def split_data(data_root, output_root, seed=42):
    ...
    for p in train_patients:
        shutil.copytree(
            os.path.join(data_root, p), os.path.join(train_dir, p), dirs_exist_ok=True
        )
    for p in val_patients:
        shutil.copytree(
            os.path.join(data_root, p), os.path.join(val_dir, p), dirs_exist_ok=True
        )
    for p in test_patients:
        shutil.copytree(
            os.path.join(data_root, p), os.path.join(test_dir, p), dirs_exist_ok=True
        )


def create_brats_dataset(
    brats_root, output_root, slice_range=(27, 127), mask_threshold=0.1, modalities=None
):
    """
    ...
    for modality in modalities:
        ...
        for split_type, list_of_cases in dataset_splits.items():
            ...
            slice_idx = 0
            for case_path in tqdm(
                list_of_cases, desc=f"Processing {modality}->{split_type}"
            ):
                # Path to a NIfTI file for the chosen modality
                nifti_file = os.path.join(
                    case_path,
                    f"{os.path.basename(case_path.strip('/'))}_{modality}.nii.gz",
                )
    ...
    for case_path in tqdm(test_cases, desc="Creating T1 masks"):
        t1_file = os.path.join(
            case_path, f"{os.path.basename(case_path.strip('/'))}_t1.nii.gz"
        )
...
def main():
    ...
    brats_root = "/mnt/sdb/zhangxiaobo/Sit/data/BraTS2021"
    output_root = "/mnt/sdb/zhangxiaobo/Sit/data/BraTS2021_SelfRDB"
