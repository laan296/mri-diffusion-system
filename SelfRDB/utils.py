...
def save_eval_images(
    source_images, target_images, pred_images, psnrs, ssims, save_path
):
    h, w = 30, 30
...
def center_crop(x, crop):
    ...
    x = x[
        ...,
        h // 2 - crop[0] // 2 : h // 2 + crop[0] // 2,
        w // 2 - crop[1] // 2 : w // 2 + crop[1] // 2,
    ]
    return x


def ax_zoomed(ax, im, zoom_region, zoom_size, zoom_edge_color="yellow"):
...
def compute_metrics(
    gt_images, pred_images, mask=None, norm="mean", subject_ids=None, report_path=None
):
    ...
    """
    # 打印调试信息
    # print(f"Entering compute_metrics")
    # print(f"gt_images shape: {gt_images.shape}, dtype: {gt_images.dtype}")
    # print(f"pred_images shape: {pred_images.shape}, dtype: {pred_images.dtype}")
    ...
    with warnings.catch_warnings():
        ...
        assert (
            gt_images.shape == pred_images.shape
        ...
        if subject_ids is not None:
            psnr_mean = np.nanmean(
                [report["psnr_mean"] for report in subject_reports.values()]
            )
            ssim_mean = np.nanmean(
                [report["ssim_mean"] for report in subject_reports.values()]
            )
            ...
            with open(report_path, "w") as f:
                ...
                if subject_ids is not None:
                    for subject_id, report in subject_reports.items():
                        ...
                        f.write(
                            f'PSNR: {report["psnr_mean"]:.2f} ± {report["psnr_std"]:.2f}\n'
                        )
                        f.write(
                            f'SSIM: {report["ssim_mean"]:.2f} ± {report["ssim_std"]:.2f}\n'
                        )
                        f.write("\n")
