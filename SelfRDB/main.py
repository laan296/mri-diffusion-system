...
class BridgeRunner(L.LightningModule):
    ...
    def training_step(self, batch):
        ...
        if self.global_step % self.disc_grad_penalty_freq == 0:
            grads = torch.autograd.grad(
                outputs=disc_out.sum(), inputs=x_t, create_graph=True
            )[0]
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 0 and self.global_rank == 0:
            path = os.path.join(
                self.logger.log_dir, "val_samples", f"epoch_{self.current_epoch}.png"
            )
    ...
    def test_step(self, batch, batch_idx):
        ...
        # print(f"Debug - x0_pred shape: {x0_pred.shape}")
        # print(f"Debug - slice_idx: {slice_idx}")
    ...
    def on_test_end(self):
        ...
        if self.global_rank == 0:
            ...
            pred_images_list = [x[1] for x in self.test_samples]

            # 检查预测图像的形状
            # print(f"Debug - Number of predictions: {len(pred_images_list)}")
            # print(
            #     f"Debug - First pred shape: {pred_images_list[0].shape if len(pred_images_list) > 0 else 'No preds'}"
            # )
            ...
            if hasattr(dataset, "target"):
                ...
                # print(
                #     f"Debug - Target shape: {target.shape if hasattr(target, 'shape') else 'No shape attr'}"
                # )
            else:
                # 如果数据集没有target属性，则从数据集中加载
                print("Loading targets from dataset...")
                target_list = []
                for i in range(len(dataset)):
                    _, target_img, _ = dataset[i]
                    target_list.append(
                        target_img.numpy()
            ...
            metrics = compute_metrics(
                ...
                report_path=os.path.join(
                    self.logger.log_dir, "test_samples", "report.txt"
                ),
            )
    ...
    def configure_optimizers(self):
        optimizer_g = Adam(
            self.generator.parameters(), lr=self.lr_g, betas=self.optim_betas
        )
        optimizer_d = Adam(
            self.discriminator.parameters(), lr=self.lr_d, betas=self.optim_betas
        )

        # Learning rate schedulers
        scheduler_g = CosineAnnealingLR(
            optimizer_g, T_max=self.trainer.max_epochs, eta_min=1e-5
        )
        scheduler_d = CosineAnnealingLR(
            optimizer_d, T_max=self.trainer.max_epochs, eta_min=1e-5
        )
...
class _LightningCLI(LightningCLI):
    def instantiate_classes(self):
        ...
        if (
            "test" in self.parser.args
            and "CSVLogger" in self.config.test.trainer.logger[0].class_path
        ):
...
def cli_main():
    ...
    torch.set_float32_matmul_precision("medium")  # 或 high、highest
